...
5. **Apply the role gate and feature rules.** Financial/work routes generally admit Manager, Admin, Super Admin and legacy Owner. Account settings require Admin or Super Admin. User mutations, analytics and account audits require Super Admin. Role comparisons lowercase the stored value. A role check does not prove that every submitted related ID belongs to that account. Known gaps include the [customer response account leak](_review/findings.md#f1), [related-ID ownership failures](_review/findings.md#f2) and [bootstrap financial-data bypass](_review/findings.md#f3). Sources: `src/endpoints/auth/jwt-auth.js:64`, `src/endpoints/auth/jwt-auth.js:94`, `src/app.js:122`, `src/app.js:170`.

The [endpoint index](README.md#endpoint-index) gives the actual gate for each route. It also includes public health/login/logout and the authenticated, retired AI catch-all. The frontend's manager gate omits legacy Owner, unlike the backend ([F37](_review/findings.md#f37)).
...
The snapshot and numbered migrations must be read together. They are not evidence of the live schema. Migration 005 removes suggestion columns still referenced at runtime; later numbered files do not restore them ([F30](_review/findings.md#f30)). See [operations](platform/operations.md#schema-sources-and-migration-inventory) for the baseline and migration rules.
...
Customer ledger locks serialize core financial writers. They do not cover every route in the system: customer deletion and pending-payment deletion have separate confirmed race paths. See [F25](_review/findings.md#f25) and [F16](_review/findings.md#f16).
...
| Payment-image Lambda | Handles the first S3 event record, reads OCR/CSV/text, redacts numeric PII, matches customers and inserts pending rows. It then archives the source and may delete the pending object. It does not itself post customer_payments. Rollback and archive failure paths are [F5](_review/findings.md#f5) and [F6](_review/findings.md#f6). See [pending payments](ledger/pending-payments.md). |
...
The [storage guide](platform/storage-and-downloads.md#4-data-model-and-s3-layout) owns full key and download rules. Invoice prefix authorization also admits own-account audit keys to broader roles than the audit API ([F4](_review/findings.md#f4)). Sources: `src/utils/downloadAuthorization.js:40`, `src/pdfCreator/zipOrchestrator.js:16`, `src/endpoints/timeTracking/timeTracking-router.js:151`, `src/endpoints/timeTracking/timeTracking-router.js:240`, `src/endpoints/timeTracking/timeTracking-router.js:1207`.
...
2. **Review work.** Managers resolve held rows, examine weekly and pre-invoice lists, and correct customer, job, date, amount or billability. Held apply does not select a retainer; legacy manual movement can. Job-family totals have a known cascade-edit inconsistency ([F9](_review/findings.md#f9)). See [billing review](invoicing/billing-review.md).
3. **Select customers and preview.** The eligibility read and invoice engine collect unbilled work, statement history, payments, write-offs and retainer state. The generation request reads a consistent snapshot before rendering. Unbilled work has no service-date ceiling, so future work can be billed even though due WIP excludes it ([F14](_review/findings.md#f14)). See [invoice engine](invoicing/create-invoice-engine.md).
