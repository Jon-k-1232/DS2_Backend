...
Source review dated 2026-09-24. Backend paths are relative to `DS2_Backend`; Lambda paths start `../DS2_Lambdas/Process_Payment_Images/`. The Lambda virtual environment was excluded. No cloud calls, database writes, or integration tests were run for this documentation.
...
| PDF preview/download | `../DS2_Frontend/src/Pages/Transactions/PendingPayments/components/PaymentPdfPreview.js:7`; retrieves a blob by exact source filename, forces PDF MIME for display, creates/revokes a browser object URL. |
...
| Success | HTTP 200 `{payments:[rawQueueRow...],pagination:{page,limit,totalItems,totalPages},message,status:200}`. |
...
| Errors | Common errors; 404 malformed/missing/foreign ID; **500** already processed, already deleted, or unexpected failure. These two state refusals are not 409 on this route. |
...
| Success | HTTP 200 `{counts,message:'File and associated pending payments deleted.',status:200}`. Soft-deletes eligible queue rows and attempts **only pending-prefix** S3 deletion. |
| Errors | Common errors; 400 missing/unsafe filename or any processed row for that exact source/account; 404 no ownership row; 500 DB/other error. S3 deletion error is logged and suppressed, so it can still return 200. |
...
The unique index is `(source_file,customer_name,payment_amount)`, without account/date/reference. To preserve two same-name/same-amount checks in one batch, the writer alters later DB filenames to `original.pdf#dup2-ref123` (or `#dup2` without reference). The actual S3 filename is not changed. This breaks exact-name file preview/group/delete association; see [F18](../_review/findings.md#f18). (`migrations/schema-snapshot-2026-09-22.sql:1542`, `../DS2_Lambdas/Process_Payment_Images/database.py:101`, `../DS2_Lambdas/Process_Payment_Images/s3_ops.py:25`.)
...
File list groups nondeleted rows with nonempty source by `source_file`: earliest extraction-row timestamp as `uploaded_at`, count rows, sum processed flags, boolean-any processed. Order is earliest timestamp descending. It never lists S3 directly. A file with zero extracted rows, a newly uploaded unprocessed object, or all rows deleted is absent. `uploaded_at` is not the browser upload metadata timestamp. (`src/endpoints/pendingPayments/pendingPayments-service.js:150`.)

Preview first proves exact-name ownership through any queue row, including deleted. It lists the processed prefix and takes the first object whose key ends with `/fileName`; no month is required. If that lookup fails or returns none, it fetches the pending-prefix exact key. It does not use queue `file_location` or a persisted exact archive key. (`src/endpoints/pendingPayments/pendingPayments-service.js:184`, `src/endpoints/pendingPayments/pendingPayments-router.js:399`.)
...
Lambda loads secrets when configured outside local stage, loads the catalog, and downloads to a temporary directory. Each successfully parsed file is put in the processed-file list **even if it yielded zero payments**. It deduplicates/matches the combined rows and writes the queue. Connection timeout is 10 seconds, statement timeout 30 seconds, idle-in-transaction timeout 60 seconds. Missing DB host causes the writer to return without writing. (`../DS2_Lambdas/Process_Payment_Images/config.py:87`, `../DS2_Lambdas/Process_Payment_Images/pipeline.py:141`, `../DS2_Lambdas/Process_Payment_Images/pipeline.py:247`, `../DS2_Lambdas/Process_Payment_Images/database.py:86`.)

The insert loop uses one connection/transaction, `ON CONFLICT DO NOTHING`, and a final commit. A row error calls **connection-wide rollback and continues**, losing earlier uncommitted good inserts; the inserted counter is not reset. This is [F5](../_review/findings.md#f5). It is not an all-or-nothing validated import, and return from this writer is not proof every extraction row is durable. (`../DS2_Lambdas/Process_Payment_Images/database.py:138`.)

For each processed file, try PDF redaction, upload to `processed_payments/<YYYY_MonthName>/<originalFilename>` (leading filename date determines month; otherwise `unknown_month`), then outside local stage delete the original pending key. The archive upload helper catches/logs errors instead of propagating them, so deletion can follow failed archive upload ([F6](../_review/findings.md#f6)). Zero-extraction files and rows lost by rollback can still be archived. Per-file exceptions earlier in parsing enter the failed list; there is no persistent failed-file row or notification written here. (`../DS2_Lambdas/Process_Payment_Images/pipeline.py:251`, `../DS2_Lambdas/Process_Payment_Images/pipeline.py:267`, `../DS2_Lambdas/Process_Payment_Images/s3_ops.py:13`, `../DS2_Lambdas/Process_Payment_Images/s3_ops.py:28`.)
...
Single soft-delete reads state, rejects processed/already deleted, then updates `deleted=true` by account/ID without a transaction, lock, or state predicate. It has a race against approval: a stale delete check can hide a row that was just posted ([F16](../_review/findings.md#f16)). No payment/retainer/invoice/S3 object is reversed or deleted by single soft-delete. (`src/endpoints/pendingPayments/pendingPayments-router.js:112`, `src/endpoints/pendingPayments/pendingPayments-service.js:101`.)

File deletion checks ownership and whether any exact-source row is processed, soft-deletes unprocessed rows, and attempts pending-key removal. These checks/writes and S3 are not one atomic transaction. It does not remove archived objects despite its success message; preview still accepts deleted ownership rows ([F17](../_review/findings.md#f17)). Suffixed source names form separate groups ([F18](../_review/findings.md#f18)). (`src/endpoints/pendingPayments/pendingPayments-router.js:337`, `src/endpoints/pendingPayments/pendingPayments-router.js:351`, `src/endpoints/pendingPayments/pendingPayments-service.js:136`, `src/endpoints/pendingPayments/pendingPayments-service.js:184`.)
...
Existing tests were read, not executed. Read-only fake-dependency checks reproduced the rollback-loss and archive-failure/delete sequences without connecting to a DB or S3.
