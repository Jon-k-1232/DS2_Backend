...
| `createdByUserID` | Ignored on update; the original creator is preserved (`src/endpoints/workDescriptions/workDescriptionsObjects.js:23`, `migrations/schema-snapshot-2026-09-22.sql:1854`). |
...
No duplicate-label guard, immutable creation timestamp, or billed-history rename gate exists in these CRUD handlers. No application limit beyond the database's text/integer types is enforced (`src/endpoints/workDescriptions/workDescriptions-router.js:13`, `src/endpoints/workDescriptions/workDescriptionsObjects.js:16`). Transaction create/update validate description account ownership before any write; fixed [F2](../_review/findings.md#f2), tested in `review-related-ids.integration.spec.js` in [consolidated findings](../_review/findings.md).
