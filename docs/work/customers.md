...
Both `/customer` and `/recurringCustomer` mounts require authentication. Every customer route installs `requireManagerOrAdmin`; the recurring router installs it for the whole router. Exact lowercase roles: `manager`, `admin`, `super admin`, `owner`. Frontend manager routing permits only the first three (`src/app.js:122`, `src/app.js:144`, `src/endpoints/customer/customer-router.js:30`, `src/endpoints/recurringCustomer/recurringCustomer-router.js:13`, `src/endpoints/auth/jwt-auth.js:94`, `../DS2_Frontend/src/Routes/ManagerAndAdminProtectedAccess.js:9`).
...
| Method | Path | Inputs | Success | Errors and triggers |
|---|---|---|---|---|
| PUT | `/customer/updateCustomer/:accountID/:userID` | Required path IDs, `{customer:{...}}`, `customerID`, `customerInfoID`, full field mapping | HTTP 200 C plus `warnings:[string]`, message `Successfully updated customer.` | Common errors; E500 for sanitization/mapping, invalid DB fields, failed recurring write or readback. No explicit 404/affected-row check: unmatched customer/contact updates can report success. Foreign body account can leak its response lists. `src/endpoints/customer/customer-router.js:239` |
...
| `accountID`, `userID` | Create trusts URL/session instead. Customer-update response still uses body account; recurring changes during update still map body creator. Contact creator is preserved. |
| `recurringCustomerID` | Optional on customer update; determines whether to create, update, or deactivate an existing recurring row. Not checked as belonging to this customer before updating it. |
...
|---|---|
| `customerID` | Create number-coerced; no owned-customer existence check on dedicated create. Update dedicated route uses stored customer ID. Embedded customer route uses submitted customer ID. |
| `recurringCustomerID` | Required update identity, number-coerced; account-scoped read required only on dedicated PUT. |
...
| `userID` | Required numeric creator FK on dedicated create/update; mapper also overwrites creator on recurring update. Embedded customer create substitutes session user. |
...
| Profile jobs | SELECT * jobs INNER JOIN types/categories; jobs account/customer; jobs created_at ASC, no completed/active predicate. Tree root total becomes child with greatest job ID's current total; no sum across snapshots (`src/endpoints/job/job-service.js:41`, `src/endpoints/customer/customer-router.js:171`). |
| Recurring list | Recurring.* plus customer display_name INNER JOIN customer ID; recurring account and active=true only. No customer-account/customer-active/start/end-date filter and no ORDER BY (`src/endpoints/recurringCustomer/recurringCustomer-service.js:3`). |
...
| UI “Payments Since Last Bill” | `paymentTotal + retainerAppliedToInvoice`. This double-counts that subset and omits invoice-tagged receipts ([F13](../_review/findings.md#f13)) (`../DS2_Frontend/src/Pages/Customer/CustomerProfile/CustomerProfile.js:238`). |
...
Example: outstanding $200 + work $150 + payments −$50 (including −$20 retainer) + write-offs −$10 = $290 due. Pre-retainer amount=$310. Unused retainer −$80 stays −$80. The current profile payment label shows −$70, although paymentTotal is −$50 ([F13](../_review/findings.md#f13); formula citations above).

### Statement PDF: activity and current amount due

The activity ledger includes all recorded work, billed and unbilled. Billable transaction is a positive charge; non-billable is a zero-charge row. Normal negative payment becomes positive credit; a positive payment reversal becomes a charge. Write-off credit is absolute amount. Root invoice issuance and root retainer establishment are informational zero-charge/zero-credit rows, avoiding duplicate work/deposit charges (`src/endpoints/accountAudit/account-audit-logic.js:441`).
...
Update performs separate autocommit customer, contact and optional recurring writes; there is no customer ledger lock or encompassing transaction. Missing contact ID or a SQL failure can leave an earlier customer update committed ([F21](../_review/findings.md#f21)). Customer/contact updates check account/IDs but ignore affected count. There is no duplicate-name check on update. Contact created-at/creator survive; customer created-at survives (`src/endpoints/customer/customer-router.js:260`, `src/endpoints/customer/customer-service.js:111`, `src/endpoints/customer/customer-service.js:124`, `src/endpoints/customer/customerObjects.js:40`).

Embedded recurring handling tests the raw recurring flag: truthy with no recurring ID inserts; falsy with positive recurring ID deactivates and stamps end_date now; truthy with positive ID updates. It does not lock/check that recurring ID's existing customer before remapping it. Deactivating the customer alone does not automatically deactivate recurring, jobs, contacts or ledger records. Warnings are emitted after any save with mapped active=false, not only on a true-to-false transition (`src/endpoints/customer/customer-router.js:264`, `src/endpoints/customer/customer-router.js:294`).

Hard customer delete first confirms owned positive integer ID. Then it queries jobs, retainers, invoices, payments, write-offs, transactions and all recurring rows. Any returned row refuses, advising disable instead, even zero/paid/non-billable/inactive records. Some guard reads use joins, so they do not count malformed rows hidden by a missing join. Checks and delete are separate statements without the shared customer ledger lock ([F25](../_review/findings.md#f25)). Quotes/contact/rate agreements are absent from these checks (`src/endpoints/customer/customer-router.js:323`, `src/endpoints/customer/customer-router.js:331`).

Delete removes only the customers row. Contact rows have no customer FK and remain ([F24](../_review/findings.md#f24)); quotes also have no customer FK. Reference-schema effects include customer rate agreements and customer audit history cascading, processed payment images' customer/matched-customer IDs becoming null, and timesheet suggested_customer_id becoming null. The `ai_time_tracker_transaction_suggestions.suggested_customer_id` FK can refuse deletion. These follow database FKs, not explicit cleanup in the route (`src/endpoints/customer/customer-service.js:132`, `migrations/schema-snapshot-2026-09-22.sql:1670`, `migrations/schema-snapshot-2026-09-22.sql:1814`, `migrations/schema-snapshot-2026-09-22.sql:1990`, `migrations/schema-snapshot-2026-09-22.sql:2046`, `migrations/schema-snapshot-2026-09-22.sql:2214`). No S3-object cleanup accompanies these FK changes.

Dedicated recurring create first sets owned customer's is_recurring=true, then independently inserts a recurring row. It ignores zero updated customers and has no transaction or duplicate check. A later insert failure can leave the flag true ([F21](../_review/findings.md#f21)); a foreign customer ID can be linked in an own-account recurring row ([F2](../_review/findings.md#f2)). Dedicated recurring PUT preserves stored customer ID but overwrites mapped fields/creator. DELETE sets is_recurring_customer_active=false and end_date now, preserving the row. PUT/DELETE do not recompute customers.is_recurring, so it can stay true after the last active subscription ends ([F22](../_review/findings.md#f22)) (`src/endpoints/recurringCustomer/recurringCustomer-router.js:26`, `src/endpoints/customer/customer-service.js:120`, `src/endpoints/recurringCustomer/recurringCustomer-router.js:90`, `src/endpoints/recurringCustomer/recurringCustomer-router.js:135`).
...
| Exact statement membership and audit/engine parity | `test/endpoints/accountAudit/statement-gate.spec.js:1`, `test/endpoints/accountAudit/engine-parity.spec.js:1`. These are calculation tests, not proof of a consistent snapshot in the customer statement route. |
...
Findings [F1](../_review/findings.md#f1), [F2](../_review/findings.md#f2), [F21](../_review/findings.md#f21), [F22](../_review/findings.md#f22), [F24](../_review/findings.md#f24), [F25](../_review/findings.md#f25), [F13](../_review/findings.md#f13) and [F27](../_review/findings.md#f27) in [consolidated findings](../_review/findings.md) cover the response-account leak, unchecked related IDs, partial saves, recurring-flag drift, contact orphans, unlocked deletion, profile payment double-counting and statement reads across changing database states. Multiple active contacts/recurring rows, missing range validation and different response envelope shapes are also observable limitations (`src/endpoints/customer/customer-service.js:67`, `src/endpoints/customer/customer-statement.js:28`).
