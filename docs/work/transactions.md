...
| `loggedForUserID` | Required create/update integer FK; no account/active-user validation. |
| `selectedGeneralWorkDescriptionID` | Required create/update integer FK; no account/active-description validation. |
...
| `quantity` | Number, numeric(10,2). Create uses `Number(value)`; update uses `Number(value) \|\| 1`, so zero/omitted/NaN becomes 1. No nonnegative or six-minute validation. |
| `unitCost` | Number, numeric(10,2); no nonnegative/finite application check or rate lookup. |
| `totalTransaction` | Number then absolute value, numeric(10,2); trusted independently of quantity/rate. Core rounds totals for funding and edit deltas. |
...
| `timesheetEntryID`, `selectedGeneralWorkDescription.general_work_description`, `category`, `aiSuggestion`, `minutes`, `entity` | Optional create-time training metadata; mapper described in section 7. Not used to recompute direct-API price. |
...
Tracker ingestion and held-entry application calculate server-side: `quantityHundredths=ceil(Number(minutes)/6)*10`; `rateCents=round(Number(rate||0)*100)`; `totalCents=round(quantityHundredths*rateCents/100)`; return quantity/100, rate/100 and total/100. These flows require finite positive duration before applying. The direct transaction API does **not** call this calculator or verify `quantity*unitCost=totalTransaction` ([F11](../_review/findings.md#f11)) (`src/endpoints/timesheets/auto-ingest-orchestrator.js:541`, `src/endpoints/timesheets/auto-ingest-orchestrator.js:589`, `src/endpoints/billingReview/billingReview-service.js:290`, `src/endpoints/transactions/sharedTransactionFunctions.js:590`).
...
See [consolidated findings](../_review/findings.md): [F2](../_review/findings.md#f2) related-ID tenant leaks, [F10](../_review/findings.md#f10) stale metadata copied into new job versions, [F11](../_review/findings.md#f11) direct-API price/increment trust, [F12](../_review/findings.md#f12) internal-work billability bypass. No current database reproduction was performed. Limits above come from source and the schema reference, not an independently inspected live schema (`migrations/README.md:1`).
