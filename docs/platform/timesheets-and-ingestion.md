...
Restart can lose queued in-process ingestion. Counts use old suggestion statuses. Default queue paging is unordered. Rate changes affect new processing; there is no historical rate agreement lookup in this pipeline. Cost cap is approximate. Migration 022 restores the three suggestion customer columns removed by 005; the supported fresh build uses the dated snapshot as baseline 018 plus 019–022 (fixed [F30](../_review/findings.md#f30)). Sources: `src/endpoints/timesheets/auto-ingest-runner.js:69`, `src/endpoints/timesheets/timesheets-service.js:79`, `src/endpoints/timesheets/auto-ingest-orchestrator.js:575`, `migrations/005.drop_ai_customer_suggestion_columns.sql:1`, `migrations/schema-snapshot-2026-09-22.sql:335`.
