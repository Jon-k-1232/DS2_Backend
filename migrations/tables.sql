...
-- =========================
-- 5. user_login (DROPPED — replaced by Google OAuth in migration 016)
-- =========================
...
-- Partial unique index on users.email (only enforced for active users).
-- Added in migration 016 alongside the user_login drop. Inactive users may have
-- empty/duplicate emails (historic data); active users must have unique emails
-- so Google OAuth lookups are deterministic.
CREATE UNIQUE INDEX users_active_email_unique_idx ON users (email) WHERE is_user_active = true;
