...
| `positive-total-payments-manifest.js` | The review package for migration 019's sign flip: every arithmetically eligible parent (stored positive total == magnitude of its tagged payments, no reversal, no foreign ownership) as a CSV plus the matching `INSERT INTO _m019_reviewed` block. The 2026-09-22 snapshot's package is kept in `manifest-2026-09-22/` and its 904 rows ship inside migration 019. | None; the migration applies the pasted block, re-verifying each row against live data. |
...
4. Review the 14 payment-total exceptions (`manifest-2026-09-22/positive-total-payments-exceptions.report.csv`) and reconcile historical payment attribution. Migration 019 flips only the 904 reviewed sign-only rows in its manifest; neither it nor these reports authorize balance changes.
